...
public class DataManager {
    ...
    private void loadConfigSpawners() {
        ...
        for (File fl : files) {
            ...
            List<SpawnerDrop> spawnerDrops = new ArrayList<SpawnerDrop>(2);
            if (file.contains("Spawner-Settings.drops")) {
                for (final String dropsName : file.getConfigurationSection("Spawner-Settings.drops").getKeys(false)) {
                    ...
                    spawnerDrops.add(new SpawnerDrop(displayItem, itemToGive, dropCommands, singleDrop, chance));
                }
            }

            Spawner spawner = new Spawner(entityType, entityName, item, type, typeTranslated, displayName, maxStack, permission, spawnerDrops, commands, requiredLevel, mcMMOExp, spawnDelay);
            cache(spawner);
        }
    }
    
    private void loadPlacedSpawners() {
        this.dataCache.setPlacedSpawners(DBConnection.getInstance().getDBManager().getPlacedSpawners());
    }
    
    public void updateTopSpawners() {
        this.dataCache.setTopSpawners(this.getTopSpawnersOrdered());
    }
    
    public void saveAllSpawnersData() {
        dataCache.getDeletedSpawners().forEach(spawnerLocation -> DBConnection.getInstance().getDBManager().deleteSpawner(spawnerLocation));
        dataCache.getDeletedSpawners().clear();

        dataCache.getPlacedSpawners().values().forEach(spawner -> {
            if (spawner == null || !spawner.isQueueUpdate()) return;
    ...
    private Map<UUID, BigInteger> getTopSpawnersOrdered() {
        ...
        dataCache.getPlacedSpawners().values().forEach(spawner -> playerSpawners.put(spawner.getOwnerUUID(), spawner.getStack().add(playerSpawners.getOrDefault(spawner.getOwnerUUID(), BigInteger.ZERO))));
